      ...
      if appID not in dataset and appID not in discarted:
        ...
        if app:
          ...
          if game['release_date'] != '':
            ...
            dataset[appID] = game
            gamesAdded += 1
            if gamesAdded >= args.limit:
              Log(INFO, f"Reached the limit of {args.limit} games. Stopping the scraper.")
              break
...
if __name__ == "__main__":
  ...
  parser.add_argument('-m', '--limit',    type=int,   default=2000,             help='Maximum number of games to retrieve')
